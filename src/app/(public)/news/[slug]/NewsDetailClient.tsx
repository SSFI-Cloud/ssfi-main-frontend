...
function renderContent(md: string): string {
  return md
    ...
    .replace(/\*(.+?)\*/g, '<em>$1</em>')
    .replace(/`(.+?)`/g, '<code class="px-1.5 py-0.5 bg-gray-100 text-violet-600 rounded text-sm font-mono">$1</code>')
    .replace(/^> (.+)$/gm, '<blockquote class="pl-4 border-l-4 border-violet-300 text-gray-600 italic my-4">$1</blockquote>')
    ...
    .replace(/\[(.+?)\]\((.+?)\)/g, '<a href="$2" class="text-violet-600 underline hover:text-violet-700" target="_blank" rel="noopener">$1</a>')
...
export default function NewsDetailClient() {
  ...
  if (notFoundFlag || !article) return (
    <div className="min-h-screen bg-[#f5f6f8] flex flex-col items-center justify-center gap-4">
      ...
      <Link href="/news" className="text-violet-600 hover:underline flex items-center gap-1"><ArrowLeft className="w-4 h-4" />Back to News</Link>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#f5f6f8]">
            ...
            <div className="absolute inset-0 flex flex-col justify-end">
              <div className="max-w-4xl mx-auto w-full px-4 pb-10 md:pb-14">
                <div className="flex items-center gap-3 mb-4">
                  {article.category && (
                    <span className="px-3 py-1 rounded-full bg-violet-600/90 text-white text-xs font-bold backdrop-blur-sm">{article.category}</span>
                  )}
                  {article.isFeatured && (
                    <span className="px-3 py-1 rounded-full bg-amber-500/90 text-white text-xs font-bold backdrop-blur-sm">Featured</span>
                  )}
                </div>
          ...
          <div className="bg-gradient-to-br from-[#0a1628] via-[#0f1d35] to-[#162d50] pt-32 pb-14 px-4">
            <div className="max-w-4xl mx-auto">
              <div className="flex items-center gap-3 mb-5">
                {article.category && <span className="px-3 py-1 rounded-full bg-violet-600 text-white text-xs font-bold">{article.category}</span>}
              </div>
              <h1 className="text-3xl md:text-5xl font-extrabold text-white leading-tight tracking-tight mb-4">{article.title}</h1>
      ...
      <div className="bg-white border-b border-gray-100 shadow-sm sticky top-0 z-20">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <Link href="/news" className="flex items-center gap-2 text-gray-500 hover:text-violet-600 transition-colors text-sm font-medium">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => share('copy')} className="p-1.5 text-gray-400 hover:text-violet-600 hover:bg-violet-50 rounded-lg transition-colors"><LinkIcon className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-8 md:p-12">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mt-8 pt-8 border-t border-gray-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => share('copy')} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-violet-50 text-violet-600 hover:bg-violet-100 text-sm font-medium transition-colors">
        ...
        {related.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-14">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/news" className="text-sm text-violet-600 hover:underline flex items-center gap-1">All News <ArrowRight className="w-3.5 h-3.5" /></Link>
            </div>
              ...
              {related.map(rel => {
                ...
                return (
                  <Link key={rel.id} href={`/news/${rel.slug}`}>
                    <div className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:border-violet-200 hover:shadow-lg hover:shadow-gray-200/50 transition-all duration-300">
                      ...
                      <div className="p-4">
                        {rel.category && <span className="text-xs text-violet-600 font-semibold uppercase tracking-wider">{rel.category}</span>}
                        <h3 className="text-sm font-bold text-gray-900 mt-1 line-clamp-2 group-hover:text-violet-600 transition-colors leading-snug">{rel.title}</h3>
