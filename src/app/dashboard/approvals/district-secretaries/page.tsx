...
export default function DistrictSecretariesApprovalPage() {
    const [secretaries, setSecretaries]           = useState<DistrictSecretary[]>([]);
    ...
    const [rejectTarget, setRejectTarget]         = useState<DistrictSecretary | null>(null);
    ...
    const fetchSecretaries = useCallback(async () => {
        ...
        try {
            ...
            const res = await api.get('/district-secretaries', { params });
            ...
            setError(err.response?.data?.message ?? 'Failed to load district secretaries');
        } finally {
            setLoading(false);
        }
    }, [filter, searchQuery]);
    ...
    const handleApprove = async (sec: DistrictSecretary) => {
        setActionLoading(sec.id);
        try {
            await api.put(`/district-secretaries/${sec.id}/status`, { status: 'APPROVED' });
    ...
    const openRejectModal = (sec: DistrictSecretary) => {
        setRejectTarget(sec);
    ...
    const handleReject = async () => {
        ...
        try {
            await api.put(`/district-secretaries/${rejectTarget.id}/status`, {
                status: 'REJECTED',
    ...
    const handleResendCredentials = async (sec: DistrictSecretary) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900">District Secretary Approvals</h1>
                    <p className="text-gray-500">Manage district secretary registrations</p>
                </div>
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-1 text-sm text-gray-500">
                                    ...
                                    <div className="flex items-center gap-2 text-gray-900 font-medium">
                                        ...
                                        {sec.district.name}, {sec.state.name}
                                    </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <p className="text-gray-500 text-center text-sm mb-6">
                                Rejecting <span className="text-gray-900 font-medium">{rejectTarget.name}</span>'s district secretary application.
                            </p>
                            <textarea
                                value={rejectReason}
