...
const ACCENT_COLORS = ['teal', 'emerald', 'cyan', 'sky', 'emerald', 'teal'];
const ACCENT: Record<string, { badge: string; iconBg: string; hover: string; gradient: string }> = {
    teal:    { badge: 'bg-teal-50 text-teal-700 border-teal-200',       iconBg: 'bg-teal-100 text-teal-600',     hover: 'hover:border-teal-200',    gradient: 'from-teal-500 to-emerald-500' },
    ...
    cyan:    { badge: 'bg-cyan-50 text-cyan-700 border-cyan-200',       iconBg: 'bg-cyan-100 text-cyan-600',     hover: 'hover:border-cyan-200',   gradient: 'from-cyan-500 to-teal-500' },
    ...
    return (
        <div className="min-h-screen bg-[#f5f6f8]">
            ...
            <section className="relative overflow-hidden bg-gradient-to-br from-[#0a1628] via-[#0f1d35] to-[#162d50]">
                ...
                <div className="absolute -top-32 -right-32 w-[400px] h-[400px] rounded-full" style={{ background: 'radial-gradient(circle, rgba(16,185,129,0.12) 0%, transparent 70%)' }} />
                    ...
                    <div className="flex flex-wrap gap-2 mb-10">
                        {categories.map(cat => (
                            <button key={cat} onClick={() => handleCat(cat)}
                                className={`px-4 py-2 rounded-full text-sm font-semibold transition-all ${selectedCategory === cat ? 'bg-emerald-600 text-white shadow-md shadow-emerald-200/50' : 'bg-white text-gray-500 border border-gray-200 hover:border-emerald-200 hover:text-emerald-600'}`}>
                                {cat}
                            </button>
                        ))}
                    </div>
                                            ...
                                            <div className={`group relative bg-white rounded-2xl border border-gray-100 overflow-hidden ${ac.hover} hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300`}>
                                                ...
                                                <div className="grid md:grid-cols-2 gap-0">
                                                    ...
                                                    <div className="p-7 md:p-9 flex flex-col justify-center">
                                                        ...
                                                        <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-3 leading-tight group-hover:text-emerald-600 transition-colors">{featured.title}</h2>
                                                        ...
                                                        <div className="flex items-center justify-between">
                                                            ...
                                                            <span className="text-emerald-600 text-sm font-bold flex items-center gap-1.5 group-hover:gap-2.5 transition-all">
                                                ...
                                                <Link href={`/news/${article.slug}`}>
                                                    <div className={`group relative bg-white rounded-2xl border border-gray-100 overflow-hidden ${ac.hover} hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300 h-full flex flex-col`}>
                                                        ...
                                                        <div className="p-5 flex flex-col flex-1">
                                                            ...
                                                            <h3 className="text-[17px] font-bold text-gray-900 mb-2 line-clamp-2 group-hover:text-emerald-600 transition-colors leading-snug">{article.title}</h3>
                                                            ...
                                                            <div className="flex items-center justify-between pt-3 border-t border-gray-100 mt-auto">
                                                                ...
                                                                <span className="text-emerald-600 text-sm font-semibold flex items-center gap-1 group-hover:gap-2 transition-all">Read <ArrowRight className="w-3.5 h-3.5" /></span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </Link>
                                ...
                                <div className="flex justify-center gap-2 mt-10">
                                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(pg => (
                                        <button key={pg} onClick={() => handlePage(pg)}
                                            className={`w-10 h-10 rounded-xl text-sm font-semibold transition-all ${pg === page ? 'bg-emerald-600 text-white shadow-md' : 'bg-white text-gray-500 border border-gray-200 hover:border-emerald-300'}`}>
                                            {pg}
                                        </button>
                                    ))}
                                </div>
            ...
            <section className="pb-20 pt-6">
                <div className="max-w-7xl mx-auto px-4">
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                        ...
                        <div className="absolute -top-20 -right-20 w-[300px] h-[300px] rounded-full" style={{ background: 'radial-gradient(circle, rgba(16,185,129,0.15) 0%, transparent 70%)' }} />
                        <div className="relative p-10 md:p-16 text-center">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Enter your email" required
                                    className="flex-1 px-5 py-3.5 bg-white/10 text-white placeholder-white/40 rounded-xl border border-white/15 focus:outline-none focus:border-emerald-400/50 backdrop-blur-sm" />
                                <button type="submit" className="px-7 py-3.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white rounded-xl font-bold hover:scale-[1.03] transition-all shadow-lg shadow-emerald-500/25">
                                    Subscribe
                                </button>
                            </form>
                        </div>
