      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="md:col-span-1 flex flex-col gap-3">
          <RenewalCountdownBadge
            ...
            accountStatus={membership.isActive ? 'ACTIVE' : 'EXPIRED'}
          />
          {(membership.needsRenewal || !membership.isActive) && (
            <Link href="/dashboard/renew-membership"
              className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-sm font-semibold transition-colors">
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <Link href="/dashboard/my-events" className="text-xs font-medium text-emerald-600 hover:underline flex items-center gap-1">
          ...
          <div className="divide-y divide-gray-50 max-h-80 overflow-y-auto">
            {eventRegistrations.length > 0 ? eventRegistrations.slice(0, 5).map((reg: any) => (
              <Link key={reg.id} href={`/dashboard/my-events/${reg.event.id}`}
                ...
                <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-50 to-teal-50 border border-emerald-100 flex flex-col items-center justify-center flex-shrink-0">
                  <span className="text-[10px] font-semibold text-emerald-500 uppercase leading-none">{new Date(reg.event.eventDate).toLocaleDateString('en-IN', { month: 'short' })}</span>
                ...
                <div className="flex flex-col items-end gap-1">
                  ...
                  {reg.status === 'CONFIRMED' && (
                    <span className="text-xs text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-lg border border-emerald-100 font-medium">e-Ticket</span>
                  )}
                </div>
              </Link>
            )) : (
              <div className="px-5 py-10 text-center">
                ...
                <Link href="/events" className="mt-3 inline-flex items-center gap-2 px-4 py-2 bg-emerald-50 text-emerald-600 rounded-xl text-xs font-semibold hover:bg-emerald-100 transition-colors">
